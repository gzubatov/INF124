...
public class ProductResource {
	...
	// parameter and the request body is provided in JSON
	@PUT
	@Path("{id}")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public Response updateTodo(@PathParam("id") int id, Product todo) {

		// Retrieve the todo that you will need to change
		Product retrievedTodo = ProductService.getProductByPid(id);

		if (retrievedTodo == null) {
			// If not found then respond with a 404 response.
			return Response.status(Response.Status.NOT_FOUND).entity("We could not find the requested resource")
					.build();
		}
		...
		// This is done so that a null value is not passed to the todo object when
		// updating it.
		if (todo.getDescription() == null) {
			todo.setDescription(retrievedTodo.getDescription());
		}

		// Same as above. We only change fields in the todo_resource when the user has
		// entered something in a request.
	...
	// This method represents a DELETE request where the id is provided as a path
	// parameter and the request body is provided in JSON
	@DELETE
	@Path("{id}")
	...
	public Response deleteTodo(@PathParam("id") int id) {
		...
		if (ProductService.deleteProduct(retrievedTodo)) {
